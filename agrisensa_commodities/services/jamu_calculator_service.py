...
class JamuCalculatorService:
    ...
    def calculate_ingredients(condition, servings=1, body_weight=60, age_group="adult"):
        ...
        # 1. Use explicit dictionary keys if available (Best Practice)
        if "water_ml" in formula and "result_ml" in formula:
            base_water = formula["water_ml"]
            base_result = formula["result_ml"]
            ...
            # 2. Fallback to more robust regex parsing
            import re
            # Extract water volumes and multiply by servings (handles "ml air" or just "ml")
            water_match = re.search(r'(\d+)\s*ml\s*(?:air|)', preparation, re.IGNORECASE)
            if water_match:
                base_water = int(water_match.group(1))
                adjusted_water = base_water * servings
                # Only replace the matched number to prevent breaking on typos
                preparation = re.sub(rf'\b{base_water}\b(\s*ml)', f'{adjusted_water}\\1', preparation, count=1, flags=re.IGNORECASE)
            
            # Also adjust the resulting volume (handles "tersisa X ml" or "menjadi X ml")
            result_match = re.search(r'(?:tersisa|menjadi)\s*(\d+)\s*ml', preparation, re.IGNORECASE)
            if result_match:
                base_result = int(result_match.group(1))
                adjusted_result = base_result * servings
                preparation = re.sub(rf'\b{base_result}\b(\s*ml)', f'{adjusted_result}\\1', preparation, flags=re.IGNORECASE)
